...
coulomb_function2::coulomb_function2(double hbarOmega_In, int shellMax_In, int Nparticles_In){
...
void coulomb_function2::generateIndexMap(){
...
void coulomb_function2::generateBasis(){
...
int coulomb_function2::checkSympqrs(int p, int q, int r, int s){
  int result = 0;
...
int coulomb_function2::checkModSympqrs(int p, int q, int r, int s){
  int result = 0;
   // check spin
  if( this->indexMap[p][2] - this->indexMap[q][2] ==
       this->indexMap[r][2] - this->indexMap[s][2] ){
...
int coulomb_function2::checkChanSym(int p, int q, int ichan){
  int result = 0;
...
int coulomb_function2::checkChanSym2(int p, int q, int chanMl, int chanSz){
  int result = 0;
...
int coulomb_function2::checkChanModSym2(int p, int q, int chanMl, int chanSz){
  int result = 0;
...
int coulomb_function2::checkChanModSym(int p, int q, int ichan){
  int result = 0;
...
void coulomb_function2::setUpTwoStateChannels(){
...
void coulomb_function2::printBasis(){
...
void coulomb_function2::deallocate(){
...
double coulomb_function2::calc_TBME(int p, int q, int r, int s, mat mapping){
  ...
  //  qi is a three integer array:  ( n_i , ml_i , ms_i )
   ...
   double qi[3] = {mapping(p,0), mapping(p,1), mapping(p,2)};
   double qj[3] = {mapping(q,0), mapping(q,1), mapping(q,2)};
   // k and l flipped
   double qk[3] = {mapping(s,0), mapping(s,1), mapping(s,2)};
   double ql[3] = {mapping(r,0), mapping(r,1), mapping(r,2)};
...
int coulomb_function2::kron_del(int i, int j){
  if(i != j){
    return 0;
  }
  return 1;
...
double coulomb_function2::specialGamma(double x){
...
double coulomb_function2::bin_coef(int n, int k){
  double result;
  result = fac_over_fac(n,k)/factorial(n-k);
  return result;
} // end bin_coef

double coulomb_function2::fac_over_fac(int a1, int a2){
  double result = 1.;
...
int coulomb_function2::factorial(int n){
