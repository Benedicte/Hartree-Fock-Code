...
{  
  ...
  cout << "Part of the Two particle matrix elements that we need" << endl;

  for(int i=0; i<fermiLevel; i++){
      for(int j = 0; j < fermiLevel; j++){
          cout << calcVpqrs(i,j,i,j,SPbasis) << " ";
      }
      cout << endl;
  }
  ...
  while(abs(energyDiff) > tolerance){   
   ...
   // if(II == 0){
        //cout << t2.get_Mod_Bundle(0,0,0,0,0,0,SPbasis);
    //}

    //Code Ive written finished
